...
// ── Icons ──────────────────────────────────────────────────────────────
const BarIcon = () => (
  <LuChartNoAxesColumn style={{height:'25px',
      width: '25px'}}/>
);
const LegalIcon = () => (
  <BsBox style={{height:'22px',
      width: '22px'}} />
);
const TechIcon = () => (
...
const RuleIcon = ({ icon, color }: { icon: Rule["icon"]; color: string }) => {
  const bg = color + "22";
  return (
    <div style={{ width: 44, height: 44, borderRadius: 10, background: bg, display: "flex", alignItems: "center", justifyContent: "center", color }}>
      {icon === "bar" && <BarIcon />}
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={S.fieldGroup}>
          ...
          <input style={S.input} value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="e.g. Executive Summary" />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={S.fieldGroup}>
            ...
            <input style={S.input} value={form.persona} onChange={(e) => set("persona", e.target.value)} placeholder="e.g. Strategic Consultant" />
          </div>
          <div style={S.fieldGroup}>
            <label style={S.label}>Tone *</label>
            <input style={S.input} value={form.tone} onChange={(e) => set("tone", e.target.value)} placeholder="e.g. Direct & Professional" />
          </div>
        </div>

        <div style={S.fieldGroup}>
          <label style={S.label}>Jargon Handling</label>
          <select style={S.input} value={form.jargonHandling} onChange={(e) => set("jargonHandling", e.target.value as JargonHandling)}>
            ...
            <div className="info-banner">
              ...
              <div>
                ...
                <div className="info-text">
                  Define how the Lucid Architect processes complex data. These rules dictate tone, jargon replacement
                ...
                <div key={rule.id} className="rule-card">
                  ...
                  <div className="card-meta">
                    ...
                    <div className="meta-row">
                      ...
                      <span className="meta-val" style={{ color: "#374151", fontWeight: 500 }}>{rule.tone}</span>
                    </div>
                    <div className="meta-row">
                      <span className="meta-key">Jargon Handling</span>
                      <span className="jargon-badge" style={jargonStyle(rule.jargonHandling)}>{rule.jargonHandling}</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {deleteConfirm !== null && (
        <div className="del-confirm" onClick={() => setDeleteConfirm(null)}>
          <div className="del-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="del-text">This action cannot be undone. The rule and all its settings will be permanently removed.</div>
            <div className="del-actions">
              <button style={S.cancelBtn} onClick={() => setDeleteConfirm(null)}>Cancel</button>
              <button style={{ ...S.saveBtn, background: "#dc2626" }} onClick={() => handleDelete(deleteConfirm)}>Delete</button>
            </div>
          </div>
        </div>
      )}
